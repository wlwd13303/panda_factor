...
class RQStockCleaner(ABC):
    ...
    def clean_metadata(self):
        try:
            logger.info("Starting metadata cleaning for RQData")
            stocks = rqdatac.all_instruments(type='CS', market='cn', date=None)
            stocks = stocks[['order_book_id', 'symbol']]
            stocks = stocks.rename(columns={'symbol': 'name'})
            stocks = stocks[stocks["name"] != "UNKNOWN"]
            stocks['symbol'] = stocks['order_book_id'].apply(get_exchange_suffix)
            stocks['expired'] = False
            stocks = stocks.drop(columns=['order_book_id'])
