...
def backfill_turnover_f(start_date: str, end_date: str):
    ...
    for i, date in enumerate(date_list):
        ...
        records = db_handler.mongo_find(
            ...
            {"date": date, "turnover_f": {"$exists": False}}
        )
        ...
        try:
            ...
            # 构建 symbol -> turnover_rate_f 的映射
            factor_data['symbol'] = factor_data['ts_code']
            turnover_map = dict(zip(factor_data['symbol'], factor_data['turnover_rate_f']))
            ...
            if upsert_operations:
                ...
                logger.info(f"  {date}: 未匹配到 turnover_rate_f 数据")
