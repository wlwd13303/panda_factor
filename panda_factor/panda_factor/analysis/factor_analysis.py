...
def factor_analysis(df_factor: pd.DataFrame, params: Params, factor_id: str = "", task_id: str = "", logger=logging.Logger) -> None:
    """
    ...
    try:
        ...
        try:
            ...
            raise
        logger.debug(msg=f"K-line data details - rows: {len(df_k_data) if df_k_data is not None else 0}, symbols: {len(df_k_data['symbol'].unique()) if df_k_data is not None else 0}")
        ...
        try:
            ...
            if params.extreme_value_processing == "标准差" or params.extreme_value_processing == "std":
                ...
                df_factor = df_factor.groupby('date', group_keys=False).apply(lambda x: ext_out_3std(x, factor_list))  # 3-sigma extreme value processing
            else:  # Default to median method
                logger.info(msg="Using ext_out_mad method for extreme value processing")
                df_factor = df_factor.groupby('date', group_keys=False).apply(lambda x: ext_out_mad(x, factor_list))  # Median extreme value processing
                
            logger.info(msg="Starting z_score processing")
            df_factor = df_factor.groupby('date', group_keys=False).apply(lambda x: z_score(x, factor_list))  # z-score standardization
        except Exception as e:
            error_msg = f"Failed to clean factor data: {str(e)}"
            logger.error(msg=error_msg, extra={"stage": "data_cleaning"})
            raise
        logger.debug(msg=f"Factor data cleaning details stage: data_cleaning, rows: {len(df_factor) if df_factor is not None else 0}")
        ...
        logger.debug(msg=f"Factor grouping details, group number: {params.group_number}, benchmark date count: {len(df_benchmark) if df_benchmark is not None else 0}")
        ...
        for f in factor_list:
            try:
                ...
                factor_obj_list.append(factor_obj)  # Create factor class object to store various backtest parameters and results
                ...
                factor_obj.set_backtest_parameters(period=params.adjustment_cycle, predict_direction=params.factor_direction,commission = 0)

                logger.debug(msg=f"Set backtest parameters: period={params.adjustment_cycle}, predict_direction={params.factor_direction}, commission=0")
